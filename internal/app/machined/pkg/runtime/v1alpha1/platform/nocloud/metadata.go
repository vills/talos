		...
		case "physical":
			...
			if ntwrk.Mac != "" {
				...
				for hostInterfaceIter.Next() {
					if !hostInterfaceIter.Value().TypedSpec().Physical() {
						continue
					}

					macAddress := hostInterfaceIter.Value().TypedSpec().PermanentAddr.String()
					if macAddress == "" {
						macAddress = hostInterfaceIter.Value().TypedSpec().HardwareAddr.String()
					}
...
func (n *Nocloud) applyNetworkConfigV2(ctx context.Context, config *NetworkConfig, st state.State, networkConfig *runtime.PlatformNetworkConfig) (bool, error) {
	...
	for name, vlan := range config.VLANs {
		...
		err = applyNetworkConfigV2Ethernet(name, vlan.Ethernet, networkConfig, &dnsIPs)
		if err != nil {
			return false, err
		}
	}
